...
class Table_List_Table_Villes extends WP_List_Table
 { 
   ...
   function column_code_insee($item)
    {
        return '<em>' . $item['code_insee'] . '</em>';
    }
    ...
    function get_columns()
    {
        $columns = array(
            ...
            'Nom_commune'     => __('Ville', 'wpbc'),
            'Code_postal'     => __('Code postal', 'wpbc'),
            'code_insee'     => __('Code Insee', 'wpbc'),
        );
        return $columns;
    }
    ...
    function prepare_items()
    {
		...
		$total_items = $wpdb->get_var("SELECT COUNT(id) FROM $table_name  WHERE Nom_commune  like '%".str_replace("'","\\'",str_replace("\\","\\\\",$search))."%'");
	...
	if($search===false){
        $this->items = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name ORDER BY $orderby $order LIMIT %d, %d",  $per_page*$paged,$per_page), ARRAY_A);
	}else{
	$this->items = $wpdb->get_results("SELECT * FROM $table_name WHERE Nom_commune  like '%".str_replace("'","\\'",str_replace("\\","\\\\",$search))."%' ORDER BY $orderby $order LIMIT ".($paged*$per_page).", ".$per_page, ARRAY_A);
...
function validate_form_villes($item)
{
    ...
    if (empty($item['code_insee'])) $messages[] = __('Code insee manquant', 'wpbc');
