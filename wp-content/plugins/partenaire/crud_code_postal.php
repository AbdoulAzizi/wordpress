...
class Table_List_Table_CP extends WP_List_Table
 { 
    ...
    function column_name($item)
    {
        ...
        return sprintf('%s %s',
            (strlen($item['code_postal'])==1 ? '0'.$item['code_postal'] : $item['code_postal']),
    ...
    function prepare_items()
    {
		...
		$this->items = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name WHERE partenaire_id = %d ORDER BY $orderby $order LIMIT %d, %d", $partenaire_id, $paged*$per_page, $per_page), ARRAY_A);
