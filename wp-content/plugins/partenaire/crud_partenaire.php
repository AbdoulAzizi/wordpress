...
class Table_List_Table extends WP_List_Table
 ...
 { 
...

    {
        ...
        $actions = array(
            ...
            'list_code_insee' =>sprintf('<a href="?page=liste_code_insee&id_partenaire=%s">%s</a>',  $item['id'],__('Code insee', 'wpbc')),
...

    {
        ...
        if ('delete' === $this->current_action()) {
            ...
            if (!empty($ids)) {
                ...
                $wpdb->query("DELETE FROM ".$wpdb->prefix."code_postal WHERE partenaire_id IN($ids)");
...

    {
		...
		$total_items = $wpdb->get_var("SELECT COUNT(id) FROM $table_name  WHERE name  like '%".str_replace("'","\\'",str_replace("\\","\\\\",$search))."%'");
		...
		if($search===false){
			...
			$this->items = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name ORDER BY $orderby $order LIMIT %d, %d",  $paged*$per_page, $per_page), ARRAY_A);
			...
			$this->items = $wpdb->get_results("SELECT * FROM $table_name WHERE name  like '%".str_replace("'","\\'",str_replace("\\","\\\\",$search))."%' ORDER BY $orderby $order LIMIT ".($paged*$per_page).", ".$per_page, ARRAY_A);
...

{
    ...
    add_submenu_page('null', __('Assigner code insee', 'wpbc'), __('Assigner code insee', 'wpbc'), 'activate_plugins', 'assign_code_insee', 'add_code_insee_page');



    add_submenu_page('null', __('Associer code insee', 'wpbc'), __('Associer code insee', 'wpbc'), 'activate_plugins', 'liste_code_insee', 'wpbc_contacts_page_handler_cp');



    add_submenu_page('partenaires', 'Import/Export des partenaires', 'Import/Export des partenaires', 8, 'importexport_des_partenaires', 'partenaire_admin_importexport');
	...
	add_submenu_page('partenaires', 'Liste des départements', 'Liste des départements', 8, 'liste_des_departements', 'wpbc_contacts_page_handler_departements');
    ...
    add_submenu_page('null', __('CP', 'wpbc'), __('CP', 'wpbc'), 'activate_plugins', 'form_departements', 'form_page_handler_departements');



    add_submenu_page('partenaires', 'Settings', 'Paramètres', 8, 'settingspartenaires', 'wpbc_contacts_page_handler_settings');
